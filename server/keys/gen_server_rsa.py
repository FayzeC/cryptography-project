...
clientPath = "C:\\ACG_code\\Assign_Code\\client\\keys\\"
server_prikey_file = filePath + "rsa_server_private.pem"
server_encrypted_prikey_file = filePath + "rsa_server_encrypted_private.pem"

print("Generating an RSA key pair...")
# Generate a 1024-bit or 2024-bit long RSA Key pair.
keypair=RSA.generate(2048)

# store the public key to public.pem (server + client)
print("Private Key stored on to  'private.pem'")
with open(filePath + "rsa_server_public.pem", 'w') as f:
    print(keypair.publickey().exportKey().decode() ,file=f)
f.close()
with open(clientPath + "rsa_server_public.pem", 'w') as f:
    print(keypair.publickey().exportKey().decode() ,file=f)
f.close()
...
server_prikey = serialization.load_pem_private_key(keypair.exportKey(), None)
...
password = get_passwd()
pem = server_prikey.private_bytes(
encoding=serialization.Encoding.PEM,
format=serialization.PrivateFormat.PKCS8,
encryption_algorithm=serialization.BestAvailableEncryption(password.encode())
)

with open(server_encrypted_prikey_file, 'w') as f:
